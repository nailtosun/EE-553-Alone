function [y itL itS fCount] = searchUI(f,penaltyf1,x0,d,maxIteration,k)
    %% USAGE
    ...
    f0 = costf(f,penaltyf1,k,x0);
    fMid = costf(f,penaltyf1,k,x0 + alpha/2*d);
    fAlpha = costf(f,penaltyf1,k,x0 + alpha*d);
    fCount = 3;
    ...
    while (maxIteration>0)
        ...
        if f0 < fMid
            alpha = alpha / 2; fMid = costf(f,penaltyf1,k,x0+(alpha/2)*d); fAlpha = costf(f,penaltyf1,k,x0+alpha*d);
            fCount = fCount + 2;
            itL = itL+1;
